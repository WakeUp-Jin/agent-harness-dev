...
**不要把所有工具一视同仁地走同一条裁剪路径**。按「完整原文能否低成本恢复」把工具分为两类：

### 路径一：确定性头部 + 文件指针（bash 等落盘类工具）

适用于全量输出**本就该落盘**的工具（典型是 bash，执行时已流式写盘，见 `references/tools/bash-tool.md`）：

- 回填内容 = 逐字头部 + 截断标记 + 文件路径，**不调用摘要模型**
- 理由：全量原文已落盘且可被读取工具逐字翻页，恢复成本已被文件覆盖；头部通常最有用（命令回显、报错），逐字头部比 LLM 摘要更可信；省掉每次都调摘要模型的延迟与成本，也避免摘要把日志里的精确数字/路径摘错

### 路径二：LLM 摘要（web、generic 等重跑恢复型工具）

适用于「只有重跑才能恢复原文、且内容适合摘要」的工具：
...
任何「非完整」的输出回填给模型时，必须带明确的截断/压缩标记，说明内容不完整以及如何取完整原文，避免模型误把裁剪结果当全量。
